...
<div className="card2  lg:w-[480px]">
      ...
      <div className="flex justify-center m-8 gap-2">
        {[...Array(Math.ceil(videos.length / videosPerPage))].map((_, index) => (
          <button key={index} onClick={() => paginate(index + 1)} className={`mx-1 italic px-3 py-1 shadow-lg shadow-cyan-400 rounded-sm ${currentPage === index + 1 ? 'bg-cyan-50 ring-1 ring-black text-black font-extrabold' : 'bg-black text-white  font-extrabold'}`}>
            {index + 1}
          </button>
        ))}
      </div>
